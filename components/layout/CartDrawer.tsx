/**
 * CartDrawer — slide-in right panel showing cart contents.
 * Renders one section per restaurant entry with per-restaurant totals.
 * A single unified promo code field applies to all eligible restaurants.
 */
...
export function CartDrawer() {
  ...
  const activePromo = cart.getActiveGlobalPromo();

  function handleApplyPromo(e: React.FormEvent) {
    e.preventDefault();
    const result = cart.applyPromoCodeGlobal(promoInput);
          ...
          <>
            <div className="flex-1 overflow-y-auto">
              {entries.map((entry) => {
                ...
                const isMinimumMet = subtotal >= entry.minimumOrder;
                const shortfall = entry.minimumOrder - subtotal;

                return (
                  <div key={entry.restaurantId} className="border-b border-gray-100 last:border-0">
                    {/* Restaurant header */}
                    <div className="flex items-center justify-between px-5 pt-4 pb-1">
                      ...
                      <button
                        onClick={() => {
                          cart.clearRestaurant(entry.restaurantId);
                          addToast(`Заказ «${entry.restaurantName}» удалён`, 'info');
                        }}
                      ...
                      {entry.items.map(({ menuItem, quantity }) => (
                        <li key={menuItem.id} className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              onClick={() => cart.decrementItem(entry.restaurantId, menuItem.id)}
                            ...
                            <button
                              onClick={() => cart.incrementItem(entry.restaurantId, menuItem.id)}
                          ...
                          <button
                            onClick={() => {
                              cart.removeItem(entry.restaurantId, menuItem.id);
                    ...
                    {/* Per-restaurant footer */}
                    <div className="px-5 pb-4 space-y-2">
                      {!isMinimumMet && (
                        <p className="text-xs text-blue-700 bg-blue-50 rounded-lg px-3 py-2">
                          До минимальной суммы: {formatPrice(shortfall)}
                        </p>
                      )}
            ...
            <div className="border-t border-gray-100 px-5 py-3">
              {activePromo ? (
                <div className="flex items-center justify-between rounded-lg bg-green-50 border border-green-200 px-3 py-2">
                  ...
                  <button
                    onClick={cart.removePromoCodeGlobal}
            ...
            <div className="border-t border-gray-200 px-5 py-4 space-y-3">
              {entries.length > 1 && (
                <div className="flex justify-between font-bold text-gray-900 text-base">
                  ...
                  <span>{formatPrice(cart.getGrandTotal())}</span>
                </div>
              )}
              <Link
...
function EmptyCart() {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-4 px-8 text-center">
      ...
      <p className="text-sm text-gray-400">Добавьте блюда из меню ресторана</p>
    </div>
  );
}